...
def main(training, apply_sparsity):
    ...
    if training:
        ...
        for epoch in range(epochs):
            ...
            if (epoch + 1) % 50 == 0:
                print('------------   Saving model   ------------')
                torch.save(encoder.state_dict(), 'results/torch_sps_encoder_KL' + str(epoch + 1) + '.pytorch')
                torch.save(decoder.state_dict(), 'results/torch_sps_decoder_KL' + str(epoch + 1) + '.pytorch')
                torch.save(sp_decoder.state_dict(), 'results/torch_sps_sp_decoder_KL' + str(epoch + 1) + '.pytorch')
                torch.save(source_enhancement.state_dict(), 'results/torch_sps_se_KL' + str(epoch + 1) + '.pytorch')
                print('------------       Done       ------------')
        ...
        encoder.load_state_dict(
            torch.load(os.path.join(output_dir, 'torch_sps_encoder_KL50.pytorch'),
                       map_location={'cuda:1': 'cuda:0'}))
        decoder.load_state_dict(
            torch.load(os.path.join(output_dir, 'torch_sps_decoder_KL50.pytorch'),
                       map_location={'cuda:1': 'cuda:0'}))
        sp_decoder.load_state_dict(
            torch.load(os.path.join(output_dir, 'torch_sps_sp_decoder_KL50.pytorch'),
                       map_location={'cuda:1': 'cuda:0'}))
        source_enhancement.load_state_dict(
            torch.load(os.path.join(output_dir, 'torch_sps_se_KL50.pytorch'),
                       map_location={'cuda:1': 'cuda:0'}))
        print('-------------      Done        -------------')
