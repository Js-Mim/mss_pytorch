...
def main(training, apply_sparsity):
    ...
    if training:
        ...
        for epoch in range(epochs):
            ...
            for index in range(totTrainFiles / numFilesPerTr):
                ...
                for batch in tqdm(range(ms.shape[0] / B)):
                    ...
                    # Sparsity term
                    if apply_sparsity:
                        sparsity_penalty = sp_decoder.ffDec.weight.norm(1.) * 1e-4
            ...
            if (epoch + 1) % 50 == 0:
                print('------------   Saving model   ------------')
                torch.save(encoder.state_dict(), 'results/torch_sps_encoder_MSE' + str(epoch + 1) + '.pytorch')
                torch.save(decoder.state_dict(), 'results/torch_sps_decoder_MSE' + str(epoch + 1) + '.pytorch')
                torch.save(sp_decoder.state_dict(), 'results/torch_sps_sp_decoder_MSE' + str(epoch + 1) + '.pytorch')
                torch.save(source_enhancement.state_dict(), 'results/torch_sps_se_MSE' + str(epoch + 1) + '.pytorch')
                print('------------       Done       ------------')
        ...
        encoder.load_state_dict(
            torch.load(os.path.join(output_dir, 'torch_sps_encoder_MSE50.pytorch'),
                       map_location={'cuda:1': 'cuda:0'}))
        decoder.load_state_dict(
            torch.load(os.path.join(output_dir, 'torch_sps_decoder_MSE50.pytorch'),
                       map_location={'cuda:1': 'cuda:0'}))
        sp_decoder.load_state_dict(
            torch.load(os.path.join(output_dir, 'torch_sps_sp_decoder_MSE50.pytorch'),
                       map_location={'cuda:1': 'cuda:0'}))
        source_enhancement.load_state_dict(
            torch.load(os.path.join(output_dir, 'torch_sps_se_MSE50.pytorch'),
                       map_location={'cuda:1': 'cuda:0'}))
        print('-------------      Done        -------------')
...
if __name__ == '__main__':
    ...
    with torch.cuda.device(0):
        ...
        apply_sparsity_penalty = True   # Whether to apply a sparsity penalty or not to the sparse decoder
        sfiltnet = main(do_training, apply_sparsity_penalty)
        #print('-------------     DNN-Test     -------------')
